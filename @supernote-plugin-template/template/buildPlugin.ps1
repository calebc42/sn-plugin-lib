    ...
    if (-not $hasNative) {
        ...
        if (Test-Path $nodeModulesDir) {
            ...
            foreach ($moduleDir in $candidateModules) {
                if ($moduleDir.Name -eq "rtn-supernote-plugin-core") { continue }
                $dirsToScan = @()
...
function Is-IgnoredModuleName {
    ...
    if ($lower -eq "rtn-supernote-plugin-core") { return $true }
...
Purpose: Scan node_modules for third-party dependencies containing Java/Kotlin sources (ignore RN official libraries and rtn-supernote-plugin-core)
Input: ProjectRoot - project root path
...
function Find-ReactPackages {
    ...
    if (Test-Path $nodeModulesDir) {
        ...
        foreach ($moduleDir in $candidateModules) {
            ...
            if ($moduleName -eq "rtn-supernote-plugin-core") { continue }
...
function Main {
    ...
    # Step 8: Scan node_modules for Java/Kotlin sources (ignore RN official libraries and rtn-supernote-plugin-core)
    ...
    if ($shouldBuildNative) {
        ...
        $excludePkgs = @("com.facebook.react.shell.MainReactPackage", "com.ratta.supernote.note.plugincore.PluginPackage")
