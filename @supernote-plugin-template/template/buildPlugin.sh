...
get_package_info() {
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        name="$(python3 -c "import json,sys;print(json.load(open(sys.argv[1]))['name'])" "$package_json")"
        desc="$(python3 -c "import json,sys;d=json.load(open(sys.argv[1]));print(d.get('description',''))" "$package_json")"
        version="$(python3 -c "import json,sys;d=json.load(open(sys.argv[1]));print(d.get('version','0.0.1'))" "$package_json")"
    else
...
is_ignored_module_name() {
    ...
    case "$lower" in
        react-native|react|rtn-supernote-plugin-core) return 0 ;;
    esac
...
find_packages_in_directory() {
    ...
    while IFS= read -r -d '' f; do
        ...
        content="$(cat "$f")"
        ext="${f##*.}"
        package_name="$(printf "%s" "$content" | sed -n 's/^[[:space:]]*package[[:space:]]\+//p' | head -n1 | tr -d ';' | tr -d '\r')"
        class_name="$(printf "%s" "$content" | sed -n 's/.*class[[:space:]]\+\([A-Za-z0-9_]\+\).*/\1/p' | head -n1 | tr -d '\r')"
        matches_class=0
...
find_manual_react_packages_from_application() {
    ...
    for dir in "${dirs[@]}"; do
        ...
        while IFS= read -r -d '' f; do
            local text
            text="$(cat "$f")"
            text="$(printf "%s" "$text" | sed -E 's#//.*$##' | sed -E ':a;N;$!ba;s#/\*.*?\*/##gs')"
            declare -A imports=()
            while IFS= read -r line; do
                if [[ "$line" =~ ^[[:space:]]*import[[:space:]]+([[:alnum:]._]+) ]]; then
                    local fq="${BASH_REMATCH[1]}"
                    local short="${fq##*.}"
                    imports["$short"]="$fq"
                fi
            done < <(printf "%s\n" "$text")
            ...
            while IFS= read -r m; do
                local name="$m" fqcn="$name"
                if [[ "$name" != *.* && -n "${imports[$name]:-}" ]]; then fqcn="${imports[$name]}"; fi
                [[ "$fqcn" =~ Package$ ]] && results+=("$fqcn")
            done < <(printf "%s" "$text" | grep -Eo '\badd\(\s*[A-Za-z0-9_\.]+\s*\(' | sed -E 's#add\(\s*([A-Za-z0-9_\.]+)\s*\(#\1#')

            while IFS= read -r m; do
                local name="$m" fqcn="$name"
                if [[ "$name" != *.* && -n "${imports[$name]:-}" ]]; then fqcn="${imports[$name]}"; fi
                [[ "$fqcn" =~ Package$ ]] && results+=("$fqcn")
            done < <(printf "%s" "$text" | grep -Eo '\b(?:packages\.)?add\(\s*new[[:space:]]+[A-Za-z0-9_\.]+' | sed -E 's#.*new[[:space:]]+([A-Za-z0-9_\.]+).*#\1#')
        done < <(find "$dir" -type f \( -name '*.kt' -o -name '*.java' \) -print0 2>/dev/null)
    done
...
get_react_packages_from_autolinking_source() {
    ...
    while IFS= read -r name; do
        local fqcn="$name"
        if [[ "$name" != *.* && -n "${imports[$name]:-}" ]]; then
            fqcn="${imports[$name]}"
        fi
        [[ "$fqcn" =~ Package$ ]] && pkgs+=("$fqcn")
    done < <(printf "%s" "$text" | grep -Eo 'new[[:space:]]+[A-Za-z0-9_\.]+' | sed -E 's#new[[:space:]]+([A-Za-z0-9_\.]+).*#\1#' | sort -u)

    if [[ -n "$exclude_raw" ]]; then
        local IFS='|'; read -r -a excludes <<< "$exclude_raw"
        local filtered=()
        for p in "${pkgs[@]}"; do
            local skip=0
...
build_android_apk() {
    ...
    local has_packages=1
    if command -v jq >/dev/null 2>&1; then
        jq -e '.reactPackages' "$gen_cfg" >/dev/null 2>&1 && has_packages=0 || has_packages=1
    elif command -v python3 >/dev/null 2>&1; then
        if python3 - <<PY
import json,sys
cfg=json.load(open(sys.argv[1]))
exit(0 if 'reactPackages' in cfg else 1)
PY
        "$gen_cfg"; then has_packages=0; else has_packages=1; fi
    fi

    if [[ "$has_packages" -ne 0 ]]; then
        write_color_output "No reactPackages in build/generated PluginConfig.json, skipping APK build" "Yellow"
        return 1
    fi
...
copy_icon_and_update_path() {
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        icon_path="$(python3 -c "import json,sys;print((json.load(open(sys.argv[1]))).get('iconPath',''))" "$root_cfg")"
    fi
...
main() {
    ...
    local project_pkgs manual_pkgs autolink_pkgs
    project_pkgs="$(find_project_react_packages "$project_root" || true)"
    manual_pkgs="$(find_manual_react_packages_from_application "$project_root" || true)"
    autolink_pkgs="$(get_react_packages_from_autolinking_source "$project_root" "" || true)"

    local all_pkgs
    all_pkgs="$(printf "%s\n%s\n%s\n" "$project_pkgs" "$manual_pkgs" "$autolink_pkgs" | awk 'NF' | sort -u)"

    local gen_dir
    gen_dir="$(ensure_build_generated_dir "$project_root")"
    local gen_cfg="$gen_dir/PluginConfig.json"
    cp "$project_root/PluginConfig.json" "$gen_cfg"

    update_plugin_config_packages "$project_root" "$gen_dir" "$all_pkgs"

    build_react_native_bundle "$project_root" "$PACKAGE_NAME" "$gen_dir"

    if test_has_android_native_code "$project_root"; then
        if build_android_apk "$project_root" "$gen_cfg"; then
            copy_apk_and_update_config "$project_root" "$gen_dir" "$gen_cfg" || true
        fi
    else
        write_color_output "No Android native code detected, skipping APK build" "Yellow"
    fi
